...
Статус: инженерная основа спецификации для [Общее ядро и платформенные оболочки](../issues/09-shared-core-and-shells.md). Весь раунд 41 принят. Ограниченные эксперименты чисел, Windows-изоляции, WPF/Rust, двух нативных редакторов и SQLite-хранения на Windows завершены; Mac-исправления подтверждены пользователем. Технологическое направление и ответственность модулей фиксируются ниже. Вопрос 09 остаётся claimed до ответа на временной контракт 42.1; прохождение бюджетов, реальный IME и полная реализация не заявляются.
...
Это выбор технической основы для дальнейшей спецификации, а не перенос одноразового parser или макета в готовый продукт. Интерфейсы ниже сохраняют принятые продуктовые правила; окончательная формулировка временных метрик вынесена в [42.1](deadline-proposal.md).
...
## Оставшееся решение и критерии реализации

Ограниченные Windows-эксперименты и [Mac-эксперимент](../issues/32-mac-core-and-editor-probe.md) завершены. Последнее созревшее продуктовое решение вопроса 09 — формулировка времени допуска ответа, показа диагностики и подтверждения остановки в 42.1. Полный язык, lazy, каталог функций, реальный IME, доступность, Mac-хранилище и нагрузочная матрица остаются критериями реализации; небольшой parser и лексический заменитель не объявляются всем вычислителем.

Rust/Cargo 1.98.1 и rust-mingw подготовлены локально без глобальной установки. Rust/Dashu собран; для C-зависимости SQLite отдельно подготовлен Zig 0.15.2 и проверен wrapper target triple. Базовая Mac-сборка подтверждена логом, следующие исправления редактора — пользователем. Предупреждение Mac rust-objcopy о libLLVM требует устранения при подготовке воспроизводимой поставки. После согласования 42.1 окончательно фиксируется временной интерфейс, затем завершается вопрос установки, обновлений и приёмки. Прежние 200 мс в performance.json пока не меняются.
