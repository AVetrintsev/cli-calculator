...
Статус: инженерный проект для [Общее ядро и платформенные оболочки](../issues/09-shared-core-and-shells.md). Весь раунд 41 принят: восстановление вычислений, повреждённых данных и совместимость форматов. Ограниченные эксперименты численного слоя и Windows-механизма изоляции завершены; реальная связь с оболочками, редактор и хранение ещё требуют проверки. Окончательный стек и прохождение бюджетов этим документом не объявляются. Принятые язык, интерфейс, история и правила данных остаются источниками требований.
...
Ограниченные Windows-эксперименты механизма изоляции и численного слоя завершены. Следующая предпосылка — связать нативную оболочку, общий Rust-контроллер и worker: проверить ABI/владение буферами, актуальность и срок ответа, непрочитанные pipe, смерть родителя и восстановление без лавины процессов. При измерении полного пути корректность ответа проверяется вместе со временем. Отдельные предпосылки — редактор/IME/Undo, запись/backup при сбое и соответствующий macOS-путь. Полный язык, lazy, каталог функций и платформенная нагрузочная матрица сохраняются в критериях реализации; отдельный численный стенд не объявляется полной приёмкой.
