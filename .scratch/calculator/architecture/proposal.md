...
Статус: инженерный проект для [Общее ядро и платформенные оболочки](../issues/09-shared-core-and-shells.md). Весь раунд 41 принят: восстановление вычислений, повреждённых данных и совместимость форматов. Ограниченные эксперименты численного слоя, Windows-изоляции, сквозной WPF/Rust-связи и WPF-редактора с проекцией завершены; хранение, настоящий IME и соответствующий macOS-путь ещё требуют проверки. Окончательный стек и прохождение бюджетов этим документом не объявляются. Принятые язык, интерфейс, история и правила данных остаются источниками требований.
...
[Отдельный Windows-стенд](../issues/27-windows-worker-boundary.md) подтвердил нативный механизм в 11 сценариях, включая непрочитанные stdout/stdin и смерть владельца Job. Фактический выход при пороге 200 мс наблюдался на 208,4330–216,5342 мс. Это обосновывает изоляцию, но не снимает проверку полного бюджета. Отрицательная проверка срока в функции допуска не подменяет измерение того, когда общий контроллер и UI действительно обработали событие. Точное соблюдение ранее принятого контракта должно быть оценено при окончательном выборе; этот проект его молча не ослабляет.

Последующий [WPF/Rust-стенд](../issues/29-rust-shell-probe.md) прошёл 16 сценариев и измерил уже реальное применение состояния к UI. На шести обычных формулах наблюдалось 0,2143–5,6770 мс от TextChanged до применения ответа после готовности worker. Timeout при пороге 200 мс применялся на 207,9857–214,9435 мс, выход процесса — на 209,8574–216,7980 мс. Это не p95 продукта и не физическая отрисовка; полный бюджет 200 мс по-прежнему не пройден. Неуспешная остановка теперь явно удерживает worker и запрещает запуск замены; успешный stop означает wait, а не попытку kill. Окончание IO-потоков учитывается отдельно от EOF/ошибки чтения.
...
Ограниченные Windows-эксперименты чисел, изоляции, сквозной связи и редакторной проекции завершены. Следующая независимая предпосылка — [запись и backup при сбое](../issues/31-storage-failure-probe.md); реальный IME, соответствующий macOS-путь и окончательное отражение границы времени сохраняются отдельно. Полный язык, lazy, каталог функций и платформенная нагрузочная матрица остаются в критериях реализации; небольшой parser и лексический заменитель не объявляются всем вычислителем. При окончательном выборе учитываются фактическая применимость UI и границы гарантий, а не только прохождение механических сценариев.

Отсутствие готового Rust на машине устранено локальной подготовкой закреплённых официальных компонентов Rust/Cargo 1.98.1 и rust-mingw в папке численного эксперимента, без глобальной установки и изменения PATH. Чистая Rust-сборка с Dashu выполнена; пригодность того же комплекта для C-зависимостей или macOS не проверялась. Инженерные решения по AST, транспорту, SQL и библиотекам остаются здесь; пользователь участвует, когда решение меняет поведение продукта. После проверки перечисленных предпосылок фиксируются стек и интерфейсы модулей, затем завершается зависимый вопрос установки и обновлений.
