...
Статус: инженерный проект для [Общее ядро и платформенные оболочки](../issues/09-shared-core-and-shells.md). Весь раунд 41 принят: восстановление вычислений, повреждённых данных и совместимость форматов. Ограниченные эксперименты численного слоя, Windows-механизма изоляции и сквозной WPF/Rust-связи завершены; редактор, хранение и соответствующий macOS-путь ещё требуют проверки. Окончательный стек и прохождение бюджетов этим документом не объявляются. Принятые язык, интерфейс, история и правила данных остаются источниками требований.
...
Ограниченные Windows-эксперименты чисел, изоляции и сквозной связи оболочка/контроллер/worker завершены. Следующие предпосылки — редактор/IME/Undo и запись/backup при сбое; соответствующий macOS-путь и окончательное отражение границы времени сохраняются отдельно. Полный язык, lazy, каталог функций и платформенная нагрузочная матрица остаются в критериях реализации; проверенный небольшой parser не объявляется всем вычислителем. При окончательном выборе учитываются фактическая применимость UI и границы гарантий, а не только прохождение механических сценариев.
