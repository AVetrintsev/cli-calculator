...
Статус: инженерный проект для [Общее ядро и платформенные оболочки](../issues/09-shared-core-and-shells.md). Весь раунд 41 принят. Ограниченные эксперименты численного слоя, Windows-изоляции, сквозной WPF/Rust-связи, WPF-редактора и SQLite-хранения при сбое завершены. Настоящий IME и соответствующий macOS-путь ещё требуют проверки. Окончательный стек и прохождение бюджетов этим документом не объявляются; принятые продуктовые правила остаются источниками требований.
...
Ограниченные Windows-эксперименты чисел, изоляции, сквозной связи, редакторной проекции и хранения завершены. Следующая предпосылка — [Общий Rust-сеанс и нативный редактор на Mac](../issues/32-mac-core-and-editor-probe.md); реальный IME и окончательное отражение границы времени сохраняются отдельно. Полный язык, lazy, каталог функций и нагрузочная матрица остаются в критериях реализации; небольшой parser и лексический заменитель не объявляются всем вычислителем. При окончательном выборе учитываются фактическая применимость UI и границы гарантий, а не только прохождение механических сценариев.

Rust/Cargo 1.98.1 и rust-mingw подготовлены локально без глобальной установки. Rust/Dashu собран; для C-зависимости SQLite отдельно подготовлен Zig 0.15.2 и проверен wrapper target triple. Mac-сборка пока не выполнена. Инженерные решения по AST, транспорту, SQL и библиотекам остаются здесь; пользователь участвует, когда решение меняет поведение продукта. После предпосылок фиксируются стек и интерфейсы модулей, затем завершается вопрос установки и обновлений.
