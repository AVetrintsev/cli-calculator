...
The user requested quick result copy: Ctrl+C in the editor with no selection and a ready unambiguous result, plus clicking the target result. Text selection retains ordinary text copying. The help.copy.quickRule fragment uses {shortcut}; macOS mapping remains pending. Copy success/error messages must reflect the actual clipboard outcome. Post-copy behavior, result formatting and ambiguity of visually grouped values remain decisions in the [copying question](../issues/06-history-and-copy.md). This does not add result captions or a new settings toggle.
