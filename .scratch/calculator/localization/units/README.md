...
When adding a language, translate each alias in its separate locale file, check key coverage, English source equality and collisions against primary symbols and other aliases. Do not create an unannounced contextual group from conflicting aliases. The min function remains available; precise function-call versus unit-token boundaries are still a grammar decision.
