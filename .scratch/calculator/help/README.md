...
The separate help.confirm.quickRule fragment records Enter confirmation accepted in copying round 3: one ready displayed answer, history, copied result and immediate empty input in the fixed field while the window stays open. It uses the same paired locale files. The operator preview does not duplicate interaction-help fragments; [History, calculation confirmation and copying](../issues/06-history-and-copy.md) holds the remaining command and failure contracts.
