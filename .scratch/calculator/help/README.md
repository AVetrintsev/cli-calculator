...
These help materials record the conversion examples, complete precedence table, whitespace rule, comparison chains and distinction between unit powers and quantity powers accepted through expression-language round 21, with lowercase lazy units, alternative answers, explicit aliases, duration sums and metre square/cube notation through contextual-units round 3, including grouping identical-looking answers, no result captions, horizontal overflow and the limited compound-unit families. They are documentation assets; the calculator has not been implemented. Separate contracts for operand types and boundary cases remain open, including partial alternatives, intermediate dimensions before cancellation, remaining duration-token boundaries and editor boundaries for automatic metre square/cube display.
